...
function Products() {
  ...
  // Update local cart
  const onUpdateUserCartItem = (productId: any, value: any) => {
    setUserCart((prevCart: any) => {
      const numericId = parseInt(productId, 10);
      const product: any = products.find((p: any) => p.id === numericId);
  ...
  const placeOrder = () => {
    ...
    const updateOrderDataCallback = (result: any) => {
      if (result.error()) {
    ...
    updateBody.fields[ORDER_DATA_FIELD_ID] = JSON.stringify({
      userCart: Object.entries(userCart).reduce(
